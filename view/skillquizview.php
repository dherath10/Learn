...
$resultar=$obq->viewResult($attempt_id, $q_id);
$rowar=$resultar->fetch(PDO::FETCH_ASSOC);
...
if($rowar['score']==0){
   ...
   //Add to skill result
   if($q==5){
   $obq->addSkillResult($row['learn_id'], $concept_id, $score, $sl, $course_id);
   }
?>
...
<ul class="pagination">
    ...
    <div class="row">
        <div class="col-md-3 col-sm-6">
             <div class="panel panel-default al">
                 <div class="panel panel-body">
                     <h4>Your Skill Level : <span class="alert-info"><?php if($q==5) { echo $sl; } ?></span></h4>
    ...
    <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:<?php echo $score; ?>%">
      <?php echo round($score,2); ?>
        ...
        <form 
            ...
            <?php if($q==5) { ?>
            action="viewcourse.php?course_id=<?php echo $course_id; ?>"
            <?php } else { ?>
            action="skillquiz.php?course_id=<?php echo $course_id; ?>&concept_id=<?php echo $concept_id; ?>&en_id=<?php echo $en_id; ?>&attempt_id=<?php echo $attempt_id; ?>&q=<?php echo $q1; ?>" 
            <?php } ?>
            method="post">
        ...
        <div class="col-md-6 col-sm-12 col-xs-12">
          ...
          <div class="row al">
                <div class="col-md-12">
                    <div >You marked : <span class="alert-info"><?php echo $rowar['ans_des']; ?></span></div> 
                    
                    <h4>Your Answer is <?php echo $str; ?></h4>
                    <h4>Your Energy Point :<big><?php echo $score; ?></big></h4>
                     <?php if($q==5) { 
                          $btn="Go to Course Content"; ?>
                    <h4 class="text-success">Finished Skill Level Test - <?php echo $rowconcept['concept_name']; ?></h4>
                     <?php }else{
                         $btn="Next";
                     } ?>
                </div>
            ...
            <div class="row al">
                <div class="col-md-12">
                    <button class="btn btn-primary"><?php echo $btn; ?></button>
                </div>
            </div>
